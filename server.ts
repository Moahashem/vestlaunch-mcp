 ...
 *   - GET/POST /api/cron/daily-lead-count → the daily Managed Agents trigger
 *
 ...
 * (https://vercel.com/docs/functions/runtimes/node-js). The vercel.json cron
 * hits /api/cron/daily-lead-count on this same server.
 */
    ...
    if (path === "/" || path === "/health") {
      ...
      res.end(
        JSON.stringify({
          ...
          endpoints: ["/api/mcp", "/api/cron/daily-lead-count"],
        }),
      );
      return;
    }
