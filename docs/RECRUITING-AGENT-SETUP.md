...
| `GMAIL_HAZEL_REFRESH_TOKEN` | Second refresh token, minted by consenting as **mo@hazelequity.com** (same Playground flow as setup A, but using the HAZEL client below — the FFL consent app is Internal-audience and 403s hazel accounts). Unlocks the hazelequity channel from the cloud — until set, that channel honestly reports UNSWEPT. | NEW (2026-08-18) |
| `GMAIL_HAZEL_CLIENT_ID` / `GMAIL_HAZEL_CLIENT_SECRET` | OAuth client "recruiting-sweep-hazel" in the hazel-org GCP project `hazel-recruiting-sweep` (created 2026-08-18 as mo@hazelequity.com; consent app "Hazel Recruiting Sweep", Internal audience). The hazel refresh token is BOUND to this client — redemption with the FFL client fails. | NEW (2026-08-18) |
...
>
> Procedure: (1) `get_recruiting_state` → read `last_run_cloud`, `last_run_browser`, `carry_forward_cloud` (your own unfinished work; `carry_forward_browser` is the Mac half’s inbox, which you write only when you have LinkedIn items to hand over — the old shared `carry_forward` key is retired). Window = since `last_run_cloud`, minimum 2 days. (2) `get_new_applicants` for website, wix, wizehire, true_analysis. hazelequity always returns unswept — report it as UNSWEPT, never as "no applicants". (3) Relevance (Mo's rule): property management / leasing / community-management experience; Leasing Agent is lenient (customer service / sales / front desk / hospitality transferable); BD/Sales bar = sales/BD/real estate; Wizehire self-selection counts as the proxy. Never invite Maintenance, VLS-remote, Executive Assistant, or other out-of-scope roles. Wix hits are often property-owner sales leads, not applicants — judge each. (4) Send via `send_recruiting_invite` only. Dedup, the do-not-contact list, and the daily cap run INSIDE the tool — if it refuses, accept the refusal, never retry with altered spellings, and report why. (5) Watchdog: if `last_run_browser` > 3 days old, `send_watchdog_alert`. (6) `update_recruiting_state` (`last_run_cloud` = now, `carry_forward_cloud`; `carry_forward_browser` only if there are LinkedIn items), then `report_recruiting_run` with a one-line summary: sent / skipped / refused / unswept channels.
>
...
- Seed state once: `last_run_cloud`, `last_run_browser`, `carry_forward_cloud` / `carry_forward_browser`, `testgorilla_boundary` (= `2026-07-19T18:58` Alfredo Enciso) from the Drive `RECRUITING-STATE.md`, then retire the Drive file (SOP pointer update).
